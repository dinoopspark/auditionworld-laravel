...
@section('main')
 <div class="player-lft">
  ...
  @foreach($video as $videos)
   <?php $url = Config::get('app.url')."/videoplayer/".$videos->vid; ?>
 <div class="vid-title">
...
<!-- <div class="lik-n-share-inr"> 
  <a style="text-decoration:none;color: #5d5d5d" href="http://www.facebook.com/share.php?u={{$url}}" onclick="return fbs_click()" target="_blank"> Share</a>
  <span class="share-bg"> </span> 
</div>
<div class="lik-n-share-inr like">{{$likeval}} Likes

  <span class="like-bg" id="{{$videos->videoid}}" userid="{{$visitor}}"> </span> 

  <span class="liked-bg"> </span> 

</div>
<div class="lik-n-share-inr"> Tag <span class="tag-bg"> </span> </div>
...
<div>
<textarea name="comments" cols="" video="{{$videos->videoid}}" id="comments" videoType= "default"  rows="" class="coment-field"></textarea>
...
<!--<a href="javascript:;" class="myButton">Post</a>-->
...
<div class="comnt-outer">
  ...
  <div id="loading"> </div>
   @if(empty($video))
    <div class="playerhome" id="myElement" style="width:100%;height:400px;border: 10px solid #CCC;border-radius:10px;">
        <a href="javascript:;">
	<img  src="/assets/images/dummy/noImageUploaded.png" height="380" width="100%">
        </a>
    </div>
    @else
...
<div class="cmnt-lft"> <img src="/assets/profile/normal_{{$comments->profile_pic}}" style="height:56px;width:56px;"> </div>
 @if(Auth::check())
...
<p>{{$comments->comment}}</p>
 ...
 @endforeach
    ...
    <!--suggested play list-->   
     ...
     @endif
      @endforeach
	<h1 class="othr-video-h1 mar-lft1">Portfolio of {{$user_name}}</h1>
     </div>
     @if(!empty($sql))
    ...
    $i = 0;
     ...
     <?php $div_number = 0; ?>
      @foreach($popular as $populars)
          ...
          <a href="/videoplayer/{{$populars->vid}}">
