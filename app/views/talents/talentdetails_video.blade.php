...
@foreach($video as $l=>$v)
 <?php  $commentcount=getComment($v->def_id,'video'); 
                                       $CommentNum=CountComment($v->def_id,'video');
                                        if(!empty($CommentNum))
                                             $comment_number=$CommentNum ; 
                                        else
                                           $comment_number=0;
          $likeNumber=GetLike($v->def_id,'video');
          $viewNumber=GetViews($v->def_id,'video');
	...
	<button type="button" class="dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-angle-down"></i> </button>
				  <ul class="dropdown-menu">
						@if(!empty($userid) && $userid==$v->videouser)
						 ...
						 @else
							 <li><a href="#" title="Report Abuse" data-toggle="modal" onclick="addSpamid({{ $v->def_id }},'video')" data-target="#report_spam" >Report Post</a></li>
						@endif
				   </ul>
</div>
          <div class="clearfix"></div>
          ...
          @if(!empty($v->video_file))
            <div class="" id="video_display" style="border: 10px solid #CCC;border-radius:10px;">
                                    </div>
                                              <?php
                                              $info = pathinfo($v->video_file);
                                                 if($v->converted == 1){
                                                       $v->video_file = 'converted_'.$v->video_file;
                                                 }
                                              ...
                                              if( $iPod || $iPhone ){
                                                      if($info["extension"] == "flv"){
                                                              $protocol = 'http://'.$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_flv.":".$v->video_file."/playlist.m3u8";
                                                      } else{
                                                              $protocol = 'http://'.$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_mp4.":".$v->video_file."/playlist.m3u8";
                                                      }
                                              }else if($Android){
                                                preg_match('/Android ?([0-9]+\.[0-9]+)/',$str,$ar1);
                                                      ...
                                                      if((float)$ar1[1]>4.0){
                                                              if($info["extension"] == "flv"){
                                                                      $protocol = "rtmp://".$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_flv.":".$v->video_file;

                                                              } else{
                                                                      $protocol = 'rtmp://'.$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_mp4.":".$v->video_file;
                                                              }
                                                      } else {
                                                              if($info["extension"] == "flv"){
                                                                      $protocol = "rtsp://".$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_flv.":".$v->video_file;
                                                              } else{
                                                                      $protocol = "rtsp://".$streaming_server_ip.":1935/".$streaming_app_name."/".$streaming_format_mp4.":".$v->video_file;
                                                              }
                                                      }
                                              }else{
                                                       if($info["extension"] == "flv"){
                                                              $protocol = 'rtmp://'.$streaming_server_ip."/".$streaming_app_name."/".$streaming_format_flv.":".$v->video_file;
                                                       } else{
                                                              $protocol = 'rtmp://'.$streaming_server_ip."/".$streaming_app_name."/".$streaming_format_mp4.":".$v->video_file;
                                                      }
                                              }
                                      ...
                                      <script type="text/javascript">
                                             jwplayer("video_display").setup({
                                               primary: "flash",
                                                 file: "{{$protocol}}",
                                                  image: "/assets/thumbnails/{{$v->thumbnail}}",
						  skin: { name: "bekle" },
                                                  autoplay : true,
                                                  width: "100%",
						  ogo: { hide: 'true' },
       ...
       @endif
          ...
          <ul class="grid-menu pull-left">
                    @if(!empty($userid))
                                <li ><a href="#"><i class="fa fa-heart-o " onclick="likecount({{ $v->id }},{{ $userid }},'video')"></i><span class="likebtn{{ $v->id }}video"> {{ $likeNumber }}</span></a></li>                                
                                @else
                                   <li ><a href="#"><i class="fa fa-heart-o " data-toggle="modal" data-target="#signin"></i>  {{ $likeNumber }} </a></li>                                
                                @endif
            <li><a data-toggle="collapse" data-parent="#accordion" href="#collapse"><i class="fa fa-comment-o"></i> <span class="fb-comments-count" data-href="http://auditionworldtv.com/talentdetails/{{ $v->def_id }}"></span> </a></li>
...
<div class="fb-comments" data-href="http://auditionworldtv.com/talentdetails/{{ $v->def_id }}" data-width="600px" data-numposts="5"></div>
           <!--   <form name="testform" >
                                              ...
                                              <div class="form-group">
                                                <textarea class="form-control" rows="2" placeholder="Write a comment" name="comment" id="textarea{{ $l }}"></textarea>
                                                 @if(!empty($userid))
                                                    ...
                                                    <input type="button" class="postcomment_btn" name="btn" value="Post Comment" onclick="testfun({{ $l }},{{ $v->id }},{{ $userid }},this.form.comment.value,'video')">
                                                  @else
...
@foreach($video_right as $r=> $vr)

 <?php  $commentcount=getComment($vr->def_id,'video'); 
                                       $CommentNum=CountComment($vr->def_id,'video');
                                        if(!empty($CommentNum))
                                             $comment_number=$CommentNum ; 
                                        else
                                           $comment_number=0;
          $likeNumber=GetLike($vr->def_id,'video');
          $viewNumber=GetViews($vr->def_id,'video');
	 $posted_string=Findate($vr->created_at); 

                                 ?>

	  <div class="side-talent-box"> <span class="pull-left col-xs-11 row"> <a href="talentdetails/{{ $vr->id }}"><img src="{{asset('assets/profile/'.$vr->profile_pic)}}" alt=""></a>
            ...
            <div class="btn-group pull-right">
              ...
              <ul class="dropdown-menu">
                <@if(!empty($userid) && $userid==$vr->video_userid)
							<li><a href="#">Delete</a></li>
						 @else
							 <li><a href="#" title="Report Abuse" data-toggle="modal" onclick="addSpamid({{ $vr->def_id }},'video')" data-target="#report_spam" >Report Post</a></li>
						@endif
              </ul>
            </div>
            <div class="clearfix"></div>
            ...
            <div class="clearfix"></div>
            <figure><a class="fancybox" rel="1" href="{{ $vr->id }}" data-fancybox-group="gallery"><span><img src="{{asset('/assets/thumbnails/'.$vr->thumbnail)}}"></span></a></figure>
            <ul class="grid-menu pull-left">
   ...
   @endforeach
          ...
          <p style="text-align:center"><a href="/view_user_profile/{{$v->videouser }}?tab=videos">View more...</a></p>
        </div>
      </div>
    </div>
  </div>
